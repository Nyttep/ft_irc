...
/*   Updated: 2023/10/02 17:31:32 by mportrai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_INVITE(Command &command, Server &server)
{
	...
	sendAll(std::string(":") + SERVERNAME + " " + server.getUser(command.getParams()[0])->getNName() + " " + command.getParams()[1] + " :You are invited to this channel\r\n", *server.getUser(command.getParams()[0]));
