...
/*   Updated: 2023/10/04 10:47:29 by mportrai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	multiple_KICK(Command &command, Server &server, std::vector<std::string> targets, size_t i)
{
	...
	{
		sendAll(ERR_USERNOTINCHANNEL(setUserAddress(*command.getSource()), command.getSource()->getNName(), targets[i], command.getParams()[0]), *command.getSource());
