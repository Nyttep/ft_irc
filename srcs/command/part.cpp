...
/*   Updated: 2023/10/04 11:42:31 by pdubois          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	multiple_PART(Command &command, Server &server, std::vector<std::string> channels, size_t i)
{
	...
	if (server.getChan(channels[i])->isOperator(command.getSource()) == true)
		nick = std::string("@") + setUserAddress(*command.getSource());
	else
		nick = command.getSource()->getNName();
	std::string message_channel = std::string(":") + nick + " PART " + channels[i];
	...
	sendAll(message_channel, *command.getSource());
