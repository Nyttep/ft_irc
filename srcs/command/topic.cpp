...
/*   Updated: 2023/10/04 10:55:13 by mportrai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_TOPIC(Command &command, Server &server)
{
	...
	if (command.getParams().size() == 1)
	{
		...
		{
			sendAll(RPL_NOTOPIC(nick, command.getSource()->getNName(), command.getParams()[0]), *command.getSource());
			std::cout << "Redirection 331" << std::endl;
		}
		else
		{
			sendAll(RPL_TOPIC(nick, command.getSource()->getNName(), command.getParams()[0], server.getChan(command.getParams()[0])->getTopic()), *command.getSource());;
	...
	else
	{
		...
		std::string	nick;
		nick = setUserAddress(*command.getSource());
		std::string f_message = std::string(":") + nick + " PRIVMSG " + command.getParams()[0] + " :Topic changed :" + server.getChan(command.getParams()[0])->getTopic() + "\r\n";
