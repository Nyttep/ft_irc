...
/*   Updated: 2023/10/03 17:16:19 by mportrai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_WHOIS(Command &command, Server &server)
{
	...
	else
	{
		...
		sendAll(RPL_WHOISUSER(command.getSource()->getNName(), command.getSource()->getNName(), command.getSource()->getUName(), command.getSource()->getHName(), command.getSource()->getRName()), *command.getSource());
	}
}